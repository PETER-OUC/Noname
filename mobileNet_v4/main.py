...
# ======================
# 数据集定义（对齐 v4：无 transform，直接 numpy -> tensor）
# ======================

class MyDataset(Dataset):
    def __init__(self, data_in, label_1):
        self.X = data_in
        self.y = label_1
        
    def __getitem__(self, index):
        # 直接转换，不做额外归一化
        # 预处理数据 shape: (N, H, W, C) -> 模型需要 (C, H, W)
...
def train():
    ...
    # ----------------------
    # 数据加载（对齐 v4：无 transform，标签用 torch.long）
    # ----------------------
    dir_path = os.getcwd()
    ...
    # 转换为tensor（对齐 v4：dtype=torch.long）
    ...
    # 创建数据集（对齐 v4：不传 transform）
    train_data = MyDataset(X_train, y_train)
    test_data = MyDataset(X_test, y_test) if X_test is not None else None
    val_data = MyDataset(X_val, y_val) if X_val is not None else None
...
# ======================
# 预测模式（对齐 predict_mobile_net_float.py：逐样本减均值 + Min-Max 到 [0,1]，float32）
# ======================
...
def process_single_model(model_path, model_name, epoch, wav_files, output_base_dir):
    ...
    for wav_file in wav_files:
        ...
        Temp_sample_num = 359
        # 对齐 predict_mobile_net_float.py：使用 float32，不再用 uint8
        X_val = np.zeros((Temp_sample_num, freq_bins, time_bins, output_channel), dtype=np.float32)
        ...
        for frame_one in range(int(frame_num)):
            ...
            if end_idx <= len(t):
                ...
                # 对齐 predict_mobile_net_float.py：
                # 逐样本减均值 + Min-Max 归一化到 [0, 1]
                ...
                if sample_idex < Temp_sample_num:
                    ...
                    X_val[sample_idex, :, :, 0] = X_temp
                    sample_idex += 1
        
        # 对齐 v4 / predict_float：Dataset 不再传 transform
        val_data = MyDataset(X_val, y_val_pred)
...
def predict(model_dir=None):
    ...
    print(f'预测归一化：逐样本减均值 + Min-Max 归一化到 [0, 1]（对齐 predict_mobile_net_float.py）')
